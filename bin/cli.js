...
function displayBanner() {
	const gradientColors = gradient([
		'#65a30d', // noiseKit lime-600
		'#84cc16' // noiseKit lime-500
	]).multiline;
...
async function run() {
	...
	displayBanner();
	p.intro(`${color.green('noiseKit')} - noiseRandom SvelteKit Starter`);
	...
	const selectedPackageConfigs = utilities.map((util) => packageConfigs[util]).filter(Boolean);

	// Show summary before starting
	p.note(
		`• App Name: ${color.green(projectName)}\n` +
			`• Adapter: ${color.green(adapter)}\n` +
			`• Drizzle ORM: ${
				drizzleConfig
					? color.green(
			...
			`• Additional Packages: ${
				utilities.length > 0
					? '\n  ' + utilities.map((pkg) => color.green(`- ${pkg}`)).join('\n  ')
					: color.green('None')
			}`,
	...
	await p.tasks([
...

		{
			...
			task: async () => {
				...
				execSilent(`npx sv add --no-git-check tailwindcss="plugins:typography"`, projectPath);
		...
		// Configure SvelteKit adapter
		{
			...
			task: async () => {
				await sleep(300);
				execSilent(`npx sv add --no-git-check sveltekit-adapter=adapter:${adapter}`, projectPath);
